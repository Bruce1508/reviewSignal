...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "analysis_status", sa.String(length=32), server_default="pending", nullable=False
        ),
        sa.Column(
            "inserted_at",
            sa.DateTime(timezone=True),
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "classifier_type IN ('llm', 'ml', 'hybrid', 'manual')",
            name="ck_review_analyses_classifier_type",
        ),
        sa.CheckConstraint(
    ...
    op.create_index(
        "ix_review_analyses_taxonomy_version",
        "review_analyses",
        ["taxonomy_version_id"],
        unique=False,
    )
    op.create_table(
        ...
        sa.CheckConstraint(
            "change_type IN ('add', 'rename', 'merge', 'split', 'move', 'delete', 'rollback')",
            name="ck_taxonomy_changes_change_type",
        ),
        sa.ForeignKeyConstraint(
            ["from_version_id"],
            ["taxonomy_versions.id"],
    ...
    op.create_index(
        "ix_taxonomy_changes_to_version", "taxonomy_changes", ["to_version_id"], unique=False
    )
    op.create_table(
        "taxonomy_nodes",
        sa.Column("id", sa.UUID(), server_default=sa.text("gen_random_uuid()"), nullable=False),
    ...
    op.create_index(
        "ix_taxonomy_nodes_version", "taxonomy_nodes", ["taxonomy_version_id"], unique=False
    )
    op.create_table(
        "anomalies",
        sa.Column("id", sa.UUID(), server_default=sa.text("gen_random_uuid()"), nullable=False),
        ...
        sa.CheckConstraint(
            "status IN ('candidate', 'accepted', 'dismissed', 'resolved')",
    ...
    op.create_index(
        "ix_anomalies_period", "anomalies", ["period_start", "period_end"], unique=False
    )
    op.create_table(
        "review_aspects",
        sa.Column("id", sa.UUID(), server_default=sa.text("gen_random_uuid()"), nullable=False),
        ...
        sa.CheckConstraint(
            "sentiment IN ('positive', 'neutral', 'negative')", name="ck_review_aspects_sentiment"
        ),
    ...
    op.create_index(
        "ix_review_aspects_analysis", "review_aspects", ["review_analysis_id"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "manual_status_override", sa.Boolean(), server_default=sa.text("false"), nullable=False
        ),
        sa.CheckConstraint(
            "status IN ('new', 'monitoring', 'resolved')", name="ck_insights_status"
        ),
        sa.ForeignKeyConstraint(
            ["anomaly_id"],
            ["anomalies.id"],
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            ...
            name="ck_insight_actions_status",
        ),
